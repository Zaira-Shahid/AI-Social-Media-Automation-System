...
4. Set the OAuth redirect URL to the Cloudflare Tunnel HTTPS callback (I will give you the exact path in Module 14). Final hosting decision is local + named Cloudflare Tunnel — see §28. The tunnel must be a **named** tunnel, since a quick tunnel's hostname changes on every restart and would break this registered redirect URI.
